module Origen
  ...
  module Componentable
    ...
    def self.origen_model_init(klass)
    ...
    def delete!(to_delete)
      @_componentable_container.delete(to_delete)
    end
    alias_method :remove!, :delete!
    ...
    # Found out the hard way that Hashes don't just implement the :each method for enumerable and move on.
    # So... to workaround this, if a method if missing we'll check if that method is both part of
    # Enumerable.instance_methods, and if its on the @_componentable_container object. If it is, we'll call that.
    # That gives us the illusion of Enumerable without have to reimplement is ourselves.
    # Note that method_missing is also a popular method to override, so actually just ship this out to a
    # Componetable class method so that objects including Componentable can still call the original method
        
    # It will be common for componentable includers to override the :add method, so split out
    # :add into independent chunks that may be useful when includer is overridding it
